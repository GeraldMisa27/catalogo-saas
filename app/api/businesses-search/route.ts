...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const businessMatchesFilters = (business: unknown) => {
      ...
      if (category) {
        const categorySlug =
          typeof b.category === "object" && b.category ? b.category.slug : undefined;
    ...
    const businesses = [...businessesByName, ...businessesFromProductDocs].map(
      (b) => ({
        ...b,
    ...
    console.error("[businesses-search API]", error);
    return Response.json({ error: "Error al obtener los negocios" }, { status: 500 });
  }
}
